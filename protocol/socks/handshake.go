	...
	case socks5.Version:
		...
		case socks5.CommandUDPAssociate:
			...
			if packetListener == nil {
				packetListener = new(net.ListenConfig)
			}
			udpConn, err = packetListener.ListenPacket(ctx, M.NetworkFromNetAddr("udp", M.AddrFromNet(conn.LocalAddr())), M.SocksaddrFrom(M.AddrFromNet(conn.LocalAddr()), 0).String())
			if err != nil {
