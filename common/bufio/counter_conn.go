...
func NewInt64CounterConn(conn net.Conn, readCounter []*atomic.Int64, writeCounter []*atomic.Int64) *CounterConn {
	return &CounterConn{
		...
		common.Map(readCounter, func(it *atomic.Int64) CountFunc {
			return func(n int64) {
				it.Add(n)
			}
		}),
		common.Map(writeCounter, func(it *atomic.Int64) CountFunc {
			return func(n int64) {
				it.Add(n)
			}
		}),
...
func NewCounterConn(conn net.Conn, readCounter []CountFunc, writeCounter []CountFunc) *CounterConn {
	return &CounterConn{NewExtendedConn(conn), readCounter, writeCounter}
}

type CounterConn struct {
	N.ExtendedConn
	readCounter  []CountFunc
	writeCounter []CountFunc
}
...
func (c *CounterConn) UnwrapReader() (io.Reader, []CountFunc) {
	return c.ExtendedConn, c.readCounter
}

func (c *CounterConn) UnwrapWriter() (io.Writer, []CountFunc) {
